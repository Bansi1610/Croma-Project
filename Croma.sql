...
select product_name, product_price as Price from product_inventory
where product_price <=8000;

#5 Select all the products with a price between 2000 and 10000 Indian Rupees.
select * from product_inventory where product_price >=2000 and product_price <=10000;
...
select * from product_inventory where product_name like "M%" and product_name like "%D";
...
select concat(product_name, " , ", product_price) as Details from product_inventory;

#13 Select the name and price in dollars (i.e. the price must be divided by 80.) 
select product_name, product_price as "in Rs.", product_price/80 as "in Dollars" from product_inventory;
...
select sum(product_price) as "total cost of products" from product_inventory where manufacturer_code = 3;

#17 Compute the number of products with a price greater than or equal to 5000.
select count(product_name) as total_products from product_inventory where product_price >=5000;

#18 Select the name and price of all products with a price larger than or equal to 5000 Indian Rupees and sort them by price (in descending order), 
...
select * from product_inventory pi inner join manufacturers m on pi.manufacturer_code = m.Manufacturer_code;

#20 Select the product name, price, and manufacturer name of all the products.
select product_name, product_price, Manufacturer_Name 
from product_inventory pi inner join manufacturers m on pi.manufacturer_code = m.Manufacturer_code;

#21 Select the average price of each manufacturer's products, showing only the manufacturer's code.
select manufacturer_code, avg(product_price) from product_inventory pi group by manufacturer_code;

#22 Select the average price of each manufacturer's products, showing the manufacturer's name.
select m.manufacturer_name, avg(pi.product_price) as avg_price
from product_inventory pi inner join manufacturers m on pi.manufacturer_code = m.manufacturer_code
...
select manufacturer_name as manufacturer, avg(product_price) as avg_price
from product_inventory pi inner join manufacturers m on pi.manufacturer_code = m.manufacturer_code 
where product_price >=5000 group by manufacturer_name;
...
select m.manufacturer_name, pi.product_Name, MAX(pi.Product_Price) as Max_Price
from product_inventory pi inner join manufacturers m on pi.manufacturer_code = m.manufacturer_code 
group by m.manufacturer_name, pi.product_Name;

#26 Add a new product: Speaker with a price 1000 INR and manufacturer code 10.
insert into Product_inventory 
values(21, "Speakers",1000,10);

select * from product_inventory;

#27 Update the name of the product "Speakers" to "Wired Speakers".
update Product_inventory set product_Name="Wired Speakers" where product_Code=21;

#28 Apply a 10% discount to all products.
select * , product_price*10/100 as Discount, product_price*90/100 as Selling_price from Product_inventory;
...
select pi.product_name, m.manufacturer_name, pi.product_price
from product_inventory pi inner join manufacturers m on pi.manufacturer_code = m.manufacturer_code order by product_price;
